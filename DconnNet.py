...
class DconnNet(nn.Module):
    def __init__(self, num_class=1):
        ...
        self.backbone = ResNet(block=BasicBlock, layers=[3, 4, 5])
        self.sde_module = SDE_module(64, 64, out_planes)
        self.fb5 = FeatureBlock(64, 32, relu=False, last=True)
        self.fb4 = FeatureBlock(32, 16, relu=False)
        self.fb3 = FeatureBlock(16, 16, relu=False)
        # self.fb2 = FeatureBlock(16, 16)

        self.gap = gap.GlobalAvgPool2D()

        self.sb1 = SpaceBlock(64, 64, 64)
        self.sb2 = SpaceBlock(64, 32, 32)
        self.sb3 = SpaceBlock(32, 16, 16)

        self.relu = nn.ReLU()

        self.final_decoder = LWdecoder(in_channels=[16, 16, 32], out_channels=16,
                                       in_feat_output_strides=(4, 8, 16), out_feat_output_stride=4,
                                       norm_fn=nn.BatchNorm2d, num_groups_gn=None)

        # self.cls_pred_conv = nn.Conv2d(64, 32, 3, 1, 1)
        self.cls_pred_conv_2 = nn.Conv2d(16, out_planes, 1)
        # self.upsample4x_op = nn.UpsamplingBilinear2d(scale_factor=2)
        self.channel_mapping = nn.Sequential(
            nn.Conv2d(64, out_planes, 3, 1, 1),
    ...
    def forward(self, x):
        ...
        #### directional Prior ####
        directional_c4 = self.channel_mapping(c4)
        mapped_c4 = F.interpolate(directional_c4, size=(23, 23), mode='bilinear', align_corners=True)
        mapped_c4 = self.direc_reencode(mapped_c4)

        d_prior = self.gap(mapped_c4)

        c4 = self.sde_module(c4, d_prior)

        c5 = self.gap(c4)

        r4 = self.sb1(c5, c4)

        d3 = self.relu(self.fb5(r4, output_size=c3.size()[2:]) + c3)
        r3 = self.sb2(self.gap(r4), d3)


        d2 = self.relu(self.fb4(r3) + c2)
        r2 = self.sb3(self.gap(r3), d2)

        d1 = self.fb3(r2) + c1

        feat_list = [d1, d2, d3, c3]

        final_feat = self.final_decoder(feat_list)

        cls_pred = self.cls_pred_conv_2(final_feat)

        out = cls_pred.view(cls_pred.size(0), -1)
...
class LWdecoder(nn.Module):
    ...
    def forward(self, feat_list: list):
        ...
        # for idx, feat in enumerate(inner_feat_list):
        #     print(f"Feature map {idx} size: {feat.size()}")
        # max_height, max_width = max([feat.size(2) for feat in inner_feat_list]), max(
        #     [feat.size(3) for feat in inner_feat_list])
        #
        # resized_feats = [
        #     F.interpolate(feat, size=(max_height, max_width), mode='bilinear', align_corners=True) if feat.size(
        #         2) != max_height or feat.size(3) != max_width else feat for feat in inner_feat_list]
        #
        # out_feat = sum(resized_feats) / len(resized_feats)

        out_feat = sum(inner_feat_list) / 3.
        return out_feat
...
class FeatureBlock(nn.Module):
    ...
    def forward(self, x, output_size=None):
        ...
        if self.last == False:
            x = self.conv_3x3(x)
        if self.scale > 1 and output_size is None:
